  ...
  return (
    <div className="mb-3.5">
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        className="flex items-center gap-1.5 text-[13px] text-[var(--text-muted)] hover:text-[var(--accent)] transition-colors cursor-pointer"
      >
      ...
      {open && (
        <div className="mt-2.5 flex flex-col gap-3 bg-[var(--surface-2)] rounded-[var(--radius-md)] px-3.5 py-3 border border-[var(--border)]">
            ...
            {keepaliveEnabled && (
              <span className="flex items-center gap-1.5 text-[13px] text-[var(--text-dim)]">
                ...
                <input
                  ...
                  placeholder="25"
                  className="w-14 h-8 bg-[var(--surface-3)] rounded-[var(--radius-sm)] px-2 text-center text-[13px] text-[var(--text)] placeholder:text-[var(--text-dim)] outline-none focus:ring-1 focus:ring-[var(--accent)]"
                />
              </span>
            )}
          </div>
          ...
          <div className="flex flex-col gap-2">
            ...
            {customI1Enabled && (
              <input
                ...
                spellCheck={false}
                className="w-full h-9 bg-[var(--surface-3)] rounded-[var(--radius-md)] px-3 text-[13px] text-[var(--text)] placeholder:text-[var(--text-dim)] outline-none focus:ring-1 focus:ring-[var(--accent)] border border-transparent"
              />
            )}
          </div>
        </div>
