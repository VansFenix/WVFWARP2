...
const FEATURES = [
  ...
  {
    icon: Layers,
    title: "Маскировка AmneziaWG",
    desc: "Сигнатуры протокола скрываются от DPI: мусорные пакеты, случайные магические заголовки H1–H4, junk-параметры.",
    tone: "pulse",
  },
  {
    icon: QrCode,
...
const QA = [
  ...
  {
    q: "Зачем нужен режим AmneziaWG?",
    a: "Системы DPI умеют узнавать WireGuard по характерным заголовкам рукопожатия. AmneziaWG добавляет мусорные пакеты (Jc, Jmin, Jmax, S1, S2) и подменяет магические константы (H1–H4), из-за чего соединение перестаёт определяться. Такой конфиг нужно импортировать в AmneziaWG-клиент — обычный WireGuard его не поймёт.",
  },
  {
    q: "Безопасно ли это?",
