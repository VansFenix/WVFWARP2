...
function quicVarint(x: number): ArrayBuffer {
  if (x < 0x40) return new Uint8Array([x]).buffer as ArrayBuffer;
  let result: Uint8Array;
  if (x < 0x4000) {
    result = new Uint8Array(2);
    new DataView(result.buffer as ArrayBuffer).setUint16(0, x, false);
    result[0] = result[0] | 0x40;
  } else if (x < 0x40000000) {
    result = new Uint8Array(4);
    new DataView(result.buffer as ArrayBuffer).setUint32(0, x, false);
    result[0] = result[0] | 0x80;
  } else {
    result = new Uint8Array(8);
    new DataView(result.buffer as ArrayBuffer).setBigUint64(0, BigInt(x), false);
...
function quicXorBuffer(dst: Bytes, src: Bytes, dstOffset: number, srcOffset: number, length: number): void {
  ...
  for (let i = 0; i < length; i++) dstU[dstOffset + i] ^= srcU[srcOffset + i];
}

async function quicHmac(key: CryptoKey | ArrayBuffer, buffer: BufferSource): Promise<ArrayBuffer> {
  const cryptoKey = key instanceof CryptoKey ? key : await globalThis.crypto.subtle.importKey('raw', key, { name: 'HMAC', hash: 'SHA-256' }, false, ['sign']);
  return globalThis.crypto.subtle.sign('HMAC', cryptoKey, buffer);
}

async function quicInitHmacKey(): Promise<void> {
  const quicSalt = new Uint8Array([0x38, 0x76, 0x2c, 0xf7, 0xf5, 0x59, 0x34, 0xb3, 0x4d, 0x17, 0x9a, 0xe6, 0xa4, 0xc8, 0x0c, 0xad, 0xcc, 0xbb, 0x7f, 0x0a]);
...
async function quicEncryptPayload(key: CryptoKey | ArrayBuffer, payload: BufferSource, iv: BufferSource, aad: BufferSource): Promise<ArrayBuffer> {
  const cryptoKey = key instanceof CryptoKey ? key : await globalThis.crypto.subtle.importKey('raw', key, { name: 'AES-GCM', length: 128 }, false, ['encrypt']);
...
async function quicDeriveHpMask(key: CryptoKey | ArrayBuffer, sample: BufferSource): Promise<ArrayBuffer> {
  const cryptoKey = key instanceof CryptoKey ? key : await globalThis.crypto.subtle.importKey('raw', key, { name: 'AES-CBC', length: 128 }, false, ['encrypt']);
...
function quicMeasureLengths(dcidLength: number, scidLength: number, tokenLength: number, pknLength: number, payloadLength: number, padto = 0): QuicLengths {
  ...
  const headerLength = baseHeaderLength + quicVarintLength(pknLength + payloadLength + paddingLength + tagLength);
...
async function quicInitial(dcid: Bytes, scid: Bytes, token: Bytes, pkn: Uint8Array<ArrayBuffer>, payload: ArrayBuffer, padto: number): Promise<ArrayBuffer> {
  ...
  quicXorBuffer(quicIv, pkn.buffer as ArrayBuffer, 12 - pkn.byteLength, 0, pkn.byteLength);
...
function quicTlsClientHelloSniOnly(sni: string): ArrayBuffer {
  const randomBytes = new Uint8Array(new ArrayBuffer(32));
