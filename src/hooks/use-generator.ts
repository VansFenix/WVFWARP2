...
export function useGenerator() {
  ...
  const toggleService = useCallback((key: string) => {
    setState((prev) => {
      ...
      return {
        ...prev,
        selectedServices,
        excludeLan: selectedServices.length ? false : prev.excludeLan,
      };
    });
  }, []);
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      const persistentKeepalive = state.keepaliveEnabled ? (parseInt(state.keepaliveValue, 10) || 25) : null;
      ...
      if (data.success && data.content) {
        setState((prev) => ({ ...prev, isLoading: false, isGenerated: true, result: data.content! }));
      } else {
        setState((prev) => ({ ...prev, isLoading: false, error: data.message || 'Ошибка генерации' }));
      }
    } catch {
      setState((prev) => ({ ...prev, isLoading: false, error: 'Ошибка сети. Попробуйте ещё раз.' }));
    }
  }, [
  ...
  const reset = useCallback(() => {
    setState((prev) => ({ ...prev, isGenerated: false, result: null, error: '' }));
  }, []);
